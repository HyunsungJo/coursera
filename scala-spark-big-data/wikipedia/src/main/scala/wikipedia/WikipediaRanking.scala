...
object WikipediaRanking {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    langs.map(lang => (lang, occurrencesOfLang(lang, rdd))).sortBy { case (k, v) => -v }
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    langs.map { lang =>
      rdd.filter(_.text.contains(lang)).map(lang -> _)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map { case (k, v) => k -> v.size }.collect().toList.sortBy(-_._2)
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val x = langs.map { lang =>
      lang -> rdd.filter(_.text.contains(lang)).count()
    }
  }
