...
import FloatOps._
  ...
  test("merge Boundaries") {
    ...
    def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
      ...
      c.maxX = math.max(a.maxX, b.minX)
      c.maxY = math.max(a.maxY, b.minY)
      c
    }

    val merged = mergeBoundaries(a, a)
    val ref = new Boundaries
    ref.maxX = 2.0F
    ref.minX = 1.0F
    ref.maxY = 2.0F
    ref.minY = 1.0F

    assert(merged.maxX == ref.maxX, s"${merged.maxX} should be 2.0f")
    assert(merged.minX == ref.minX, s"${merged.minX} should be 1.0f")
    assert(merged.maxY == ref.maxY, s"${merged.maxY} should be 2.0f")
    assert(merged.minY == ref.minY, s"${merged.minY} should be 1.0f")
  }


  test("Leaf.insert(b)") {
    val leaf = Leaf(20f, 25f, 10f, Seq())
//    println(s"20 x 25")
    val b = new Body(3f, 22.5f, 27.5f, 0f, 0f)
    val inserted = leaf.insert(b)
    inserted match {
      case Fork(nw, ne, sw, se) =>
        assert(ne.getClass == Leaf.getClass, s"${ne.getClass} should be ${Leaf.getClass}")
      case _ =>
